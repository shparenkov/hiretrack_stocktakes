...
async function withRetryOn503(fn, maxAttempts = 3) {
...
async function checkHiretrackAvailability(input) {
    ...
    const raw = await withRetryOn503(() => availabilityLimiter.run(() => requestJson('POST', url, config.hiretrack?.headers || {}, Number(config.poller?.timeoutMs || 15000))));
