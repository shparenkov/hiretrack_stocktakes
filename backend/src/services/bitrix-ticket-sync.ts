...
interface TicketsBitrixConfig {
  ...
  fields?: {
    ...
    barcodeRaw?: string;
    barcodeType?: string;
    hiretrackItemRef?: string;
    hiretrackEqlistName?: string;
    hiretrackJobRef?: string;
    hiretrackTicketNumber?: string;
    status?: string;
  };
}

interface HiretrackConfigShape {
  poller?: { timeoutMs?: number };
  bitrix: {
    baseUrl: string;
    tickets?: TicketsBitrixConfig;
    modes?: {
      test?: {
        entityTypeId?: number;
      };
...
function resolveHiretrackRoot() {
  ...
  const matched = candidates.find((candidate) =>
    fs.existsSync(path.join(candidate, 'hiretrack.config.json'))
  );

  return matched || candidates[candidates.length - 1];
}

function loadConfig(): HiretrackConfigShape {
  const configPath = path.join(resolveHiretrackRoot(), 'hiretrack.config.json');
  return JSON.parse(fs.readFileSync(configPath, 'utf8')) as HiretrackConfigShape;
}

function requestJson(method: string, targetUrl: string, body: unknown, timeoutMs: number) {
...
function mapFields(ticket: TicketRecord, config: ReturnType<typeof getTicketsBitrixConfig>) {
...
export async function createBitrixTicketItem(ticket: TicketRecord) {
  ...
  const response = await requestJson(
    'POST',
    `${config.baseUrl}crm.item.add.json`,
    {
