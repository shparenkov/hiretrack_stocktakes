...
export interface CreateEqlistResult {
  eqlistId: number;
  jobId: number;
}
...
export async function createHiretrackEqlist(jobId: number, title: string, dateFrom: string, dateTo: string): Promise<CreateEqlistResult> {
  const { eqlistId } = await runHiretrackOdbcWrite<{ eqlistId: number }>({
