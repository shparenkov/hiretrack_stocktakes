...
// password-auth.ts) - only covers /crew-bookings and /api/crew-bookings.
// Separate from the main session on purpose: anyone with the shared
// stocktakes-app password can already reach Stock Check/Tickets/Daybook,
// but crew assignment writes into production HireTrack, so this page gets
// its own short PIN as an extra deliberate step before use.

const COOKIE_NAME = 'crew_bookings_pin';
const DEFAULT_SESSION_DAYS = 30;
const FAILURE_WINDOW_MS = 15 * 60 * 1000;
const MAX_FAILURES = 5;
const PIN_PATH = '/crew-bookings/pin';

function sessionTtlMs(): number {
  const configuredDays = Number(process.env.CREW_BOOKINGS_PIN_SESSION_DAYS || DEFAULT_SESSION_DAYS);
...
function readPin(): string {
  return (process.env.CREW_BOOKINGS_PIN || '3359').trim();
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    header { padding: 20px 24px 16px; background: #6b2d8f; color: #fff; }
    h1 { margin: 0; font-size: 20px; font-weight: 600; }
    header p { margin: 6px 0 0; color: #ecdcf5; font: 13px Arial, sans-serif; }
    ...
    input:focus { outline: 2px solid #b98ad6; outline-offset: 1px; }
    button { width: 100%; height: 42px; margin-top: 14px; border: 0; background: #6b2d8f; color: #fff; font: 600 14px Arial, sans-serif; cursor: pointer; }
    button:hover { background: #55236f; }
    .error { margin: 0 0 14px; padding: 9px 10px; background: #fae7e4; color: #932b20; font: 13px Arial, sans-serif; }
  </style>
</head>
<body>
  <main>
    <header><h1>Crew Bookings</h1><p>Введите PIN-код доступа</p></header>
...
export function installCrewBookingsPinAuth(app: import('express').Express): void {
  const pin = readPin();
  const sessionDurationMs = sessionTtlMs();
  const failures = new Map<string, { count: number; resetAt: number }>();

  app.get(PIN_PATH, (req, res) => {
    if (hasValidPinSession(req, pin)) return res.redirect('/crew-bookings/');
  ...
  app.post(PIN_PATH, (req, res) => {
    ...
    return res.redirect('/crew-bookings/');
  });

  app.use((req: Request, res: Response, next: NextFunction) => {
    const gated = req.path.startsWith('/crew-bookings') || req.path.startsWith('/api/crew-bookings');
    if (!gated || req.path === PIN_PATH) return next();
    if (hasValidPinSession(req, pin)) return next();
    if (req.path.startsWith('/api/crew-bookings')) {
