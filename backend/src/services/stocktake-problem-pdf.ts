...
export type StocktakeProblemKind = 'missing' | 'repair' | 'inactive';

export interface StocktakeProblemPdfRow {
  ...
  kind: StocktakeProblemKind;
}

export interface StocktakeProblemPdfInput {
  latestSession: string;
  previousSession: string;
  generatedAt: string;
  rows: StocktakeProblemPdfRow[];
}

export type StocktakeSummaryLevel = 'ok' | 'minor' | 'zero' | 'low' | 'critical';

export interface StocktakeSummaryPdfRow {
  equipmentType: string;
  ...
  writtenOff: number;
  sold: number;
  level: StocktakeSummaryLevel;
}

export interface StocktakeSummaryPdfInput {
  latestSession: string;
  previousSession: string;
  generatedAt: string;
  category: string;
...
function createFonts(): TFontDictionary {
  ...
  return {
    ReportFont: {
      ...
      italics: findFont(
        path.join(windowsFonts, 'segoeuii.ttf'),
        '/usr/share/fonts/truetype/dejavu/DejaVuSans-Oblique.ttf',
        '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',
      ),
      bolditalics: findFont(
        path.join(windowsFonts, 'segoeuiz.ttf'),
...
function summaryCard(value: number, label: string, accent: string): Content {
  return {
    table: {
      widths: [4, '*'],
      body: [[
...
function problemColors(kind: StocktakeProblemKind) {
  if (kind === 'repair') return { fill: COLORS.repair, accent: COLORS.repairAccent };
  if (kind === 'inactive') return { fill: COLORS.inactive, accent: COLORS.inactiveAccent };
...
function cell(
  primary: string,
  secondary = '',
  options?: { bold?: boolean; code?: boolean },
): ContentStack & TableCellProperties {
  const stack: Content[] = [{
    text: primary || '—',
    bold: options?.bold,
...
function buildTableBody(rows: StocktakeProblemPdfRow[]): TableCell[][] {
  const header = ['Описание', 'Barcode', 'Serial', 'Инвентаризация', 'Текущий статус', 'Последнее наблюдение'].map(
    (label): ContentText & TableCellProperties => ({
      text: label,
      bold: true,
      color: '#FFFFFF',
  ...
  return [
    ...
    ...rows.map((row) => {
      ...
      const cells: Array<ContentStack & TableCellProperties> = [
      ...
      cells.forEach((tableCell, index) => {
        ...
        if (index === 0) tableCell.borderColor = [colors.accent, COLORS.line, COLORS.line, COLORS.line];
      });
      return cells;
    }),
  ];
...
function buildDefinition(input: StocktakeProblemPdfInput): TDocumentDefinitions {
  ...
  const titleCell: ContentStack & TableCellProperties = {
    stack: titleStack,
    fillColor: COLORS.navy,
    margin: [16, 12, 16, 12],
    border: [false, false, false, false],
...
export async function renderStocktakeProblemPdf(input: StocktakeProblemPdfInput) {
  const printer = new PdfPrinter(createFonts());
...
function summaryLevelColor(level: StocktakeSummaryLevel) {
  if (level === 'ok') return COLORS.summaryOk;
  if (level === 'minor') return COLORS.summaryMinor;
  if (level === 'zero') return COLORS.summaryZero;
  if (level === 'low') return COLORS.summaryLow;
  return COLORS.summaryCritical;
}

function summaryTextCell(
  text: string,
  fillColor: string,
  options?: { bold?: boolean; alignment?: 'left' | 'right' | 'center' },
): ContentText & TableCellProperties {
  return {
    text,
    fillColor,
    bold: options?.bold,
...
function buildSummaryTableBody(rows: StocktakeSummaryPdfRow[]): TableCell[][] {
  const labels = [
    'Описание', 'Всего', 'Barcode', 'Активно', 'Ремонт',
    'Видели ранее', 'Видели сейчас', 'Не видели', 'Списано', 'Продано',
  ];
  const header = labels.map((label, index): ContentText & TableCellProperties => ({
    text: label,
    bold: true,
    color: '#FFFFFF',
    fillColor: COLORS.navy,
...
function buildSummaryDefinition(input: StocktakeSummaryPdfInput): TDocumentDefinitions {
...
export async function renderStocktakeSummaryPdf(input: StocktakeSummaryPdfInput) {
  const printer = new PdfPrinter(createFonts());
