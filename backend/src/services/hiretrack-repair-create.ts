...
export interface CreateHiretrackRepairInput {
  itemRef: number;
  badEqlistId?: number | null;
  faultDescription?: string | null;
  engineerNotes?: string | null;
  reportedBy?: string | null;
}

export interface HiretrackLoggedFaultCreateResult {
...
function toWin1251ChrExpression(value: string): string {
  if (!value) {
    return '';
  }

  const bytes: Buffer = iconv.encode(value, 'win1251');
  return Array.from(bytes)
    .map((byte) => `CHR(${byte})`)
    .join('+');
}

export async function createLoggedFaultInHiretrack(
  input: CreateHiretrackRepairInput,
): Promise<HiretrackLoggedFaultCreateResult> {
